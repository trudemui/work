        ...
        <div class="w-11/12 mt-12 bg-white-normal">
            <validation-observer ref="observer" v-slot="{ handleSubmit }">
                <div class="flex justify-center items-center h-14 bg-blue-normal">
                    <h3 class="text-xl sm:text-2xl font-bold text-white-normal">ホーム 新規追加</h3>
                </div>
                    ...
                    <ul class="w-full flex flex-col items-center">
                        ...
                        <li class="w-full sm:w-auto mb-8">
                            ...
                            <validation-provider rules="select_required" v-slot="{ errors }">
                                <v-select class="w-full sm:w-110 md:w-120 lg:w-130 h-10 py-0.5 shadow-md appearance-none rounded" label="farm_number" :options="farmOptions" v-model="farm_number" data-vv-validate-on="change|input">
                        ...
                        <li class="w-full sm:w-auto mb-8">
                            ...
                            <validation-provider rules="select_required" v-slot="{ errors }" class="flex flex-col">
                                <input type="text" class="w-full sm:w-110 md:w-120 lg:w-130 h-10 px-3 py-1 shadow-md appearance-none rounded truncate focus:outline-none focus:shadow-outline" oninput="value = value.replace(/[^0-9]+/i,'');" v-model="cultivatioin_area">
                                <span id="error" class="mt-1 text-red-normal">{{ errors[0] }}</span>
                            </validation-provider>
                        </li>
                        <li class="w-full sm:w-auto mb-8">
                            ...
                            <validation-provider rules="select_required" v-slot="{ errors }">
                                <v-select class="w-full sm:w-110 md:w-120 lg:w-130 h-10 py-0.5 shadow-md appearance-none rounded" label="crop_name" :options="cropOptions" v-model="crop_name" @input="find_varieties">
                                    <span slot="no-options">検索候補がありません。</span>
                                </v-select>
                                <span id="error" class="mt-1 text-red-normal">{{ errors[0] }}</span>
                            </validation-provider>
                        ...
                        <li class="w-full sm:w-auto mb-8">
                            ...
                            <validation-provider rules="select_required" v-slot="{ errors }">
                                <v-select class="w-full sm:w-110 md:w-120 lg:w-130 h-10 py-0.5 shadow-md appearance-none rounded" label="variety_name" :options="varietyOptions" v-model="variety_name" @input="delete_varieties">
                                    <span slot="no-options">検索候補がありません。</span>
                                </v-select>
                                <span id="error" class="mt-1 text-red-normal">{{ errors[0] }}</span>
                            </validation-provider>
...
export default {
    ...
    computed: {
        //ハンバーガーメニュー判定
        isActiveSidebar() {
            console.log("HomeAdd: isActiveSidebar");
            return this.$store.state.isActiveSidebar;
        }
    },
    created() {
        console.log("HomeAdd: created");
        axios
            .get("/api/growing_crops/get_list")
            .then(response => {
                console.log("HomeAdd: api get_list OK");
                //作物名
            ...
            .catch(error => {
                console.log("HomeAdd: api get_list NG");
            })
    },
    methods: { 
        //作物名に紐づいた品種名を取得
        find_varieties() {
            console.log("HomeAdd: find_varieties");
            if(this.crop_name !== null) {
                let value = { id: this.crop_name.id };
                axios
                    .post("/api/varieties/find_varieties", value)
                    .then(response => {
                        console.log("HomeAdd: api find_varieties OK");
                    ...
                    .catch(error => {
                        console.log("HomeAdd: find_varieties NG");
                    });
            } else {
                console.log("HomeAdd: find_varieties options delete!");
                this.variety_name = "";
                this.varietyOptions = [];
            }
        },
        ...
        delete_varieties() {
            ...
            if(this.crop_name !== null && this.variety_name == null) {
        ...
        //新規登録
        addDatas() {
            console.log("HomeAdd: addDatas");
            let values = {
                user_id: 1,
                variety_id: this.variety_name.id,
                starting_date: this.starting_date,
                farm_id: this.farm_number.id,
                cultivation_area: this.cultivatioin_area
            };
            axios
                .post("/api/growing_crops", values)
                .then(response => {
                    console.log("HomeAdd: api growing_crops OK");
                    this.$router.push("/Home");
                    this.scrollToTop();
                })
                .catch(error => {
                    console.log("HomeAdd: api growing_crops NG");
                });
        },
        //日付のフォーマット
        pickerClosed() {
            console.log("HomeAdd: pickerClosed");
            this.starting_date = moment(this.starting_date).format("YYYY/MM/DD");
        },
        //前の画面へ戻る
        returnPage() {
            console.log("HomeAdd: returnPage");
            this.$router.push("/home");
            this.scrollToTop();
        }
    },
