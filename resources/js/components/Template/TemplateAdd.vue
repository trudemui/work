        ...
        <div class="w-11/12 mt-12 bg-white-normal">
            <validation-observer ref="observer" v-slot="{ handleSubmit }">
                <div class="flex justify-center items-center h-14 bg-blue-normal">
                    <h3 class="text-xl sm:text-2xl font-bold text-white-normal">テンプレート 新規登録</h3>
                </div>
                    ...
                    <ul class="w-full flex flex-col items-center">
                        ...
                        <li class="w-full sm:w-auto mb-8">
                            ...
                            <validation-provider class="flex flex-col" rules="select_required" v-slot="{ errors }">
                                <input type="text" class="w-full sm:w-110 md:w-120 lg:w-130 h-10 py-2 px-3 shadow-md appearance-none rounded focus:outline-none focus:shadow-outline" v-model="selected4">
                                <span id="error" class="mt-1 text-red-normal">{{ errors[0] }}</span>
                            </validation-provider>
                        </li>
                    </ul>
...
export default {
    ...
    computed: {
        //ハンバーガーメニュー判定
        isActiveSidebar() {
            console.log("TemplateAdd: isActiveSidebar");
            return this.$store.state.isActiveSidebar;
        }
    },
    created() {
        console.log("TemplateAdd: created");
        axios
            .get("/api/templates/show_table")
            .then(response => {
                console.log("TemplateAdd: api show_table OK");
                ...
                this.options3 = values;
            })
            .catch(error => {
                console.log("TemplateAdd: api show_table NG");
            });
    },
    methods: { 
        //登録
        addDatas() {
            console.log("TemplateAdd: addDatas");
            if(this.selectedDatas.article.id === 7) {
                this.values.push({
                    crop_name: this.selected3,
                    variety_name: this.selected4
                })
                console.log(this.values[0]);
                axios
                    .post("/api/varieties", this.values[0])
                    .then(response => {
                        console.log("TemplateAdd: api varieties OK");
                        this.$router.push({
                            name: "template", 
                            params: { 
                    ...
                    .catch(error => {
                        console.log("TemplateAdd: api varieties NG");
                    });
            } else if(this.selectedDatas.article.id !== 7) {
                this.values.push({
                    category_id: this.selectedDatas.article.id,
                    body: this.selected4
                })
                axios
                    .post("/api/templates", this.values[0])
                    .then(response => {
                        console.log("TemplateAdd: api templates OK");
                        this.$router.push({
                            name: "template", 
                            params: { 
                                selectedDatas: this.selectedDatas,
                    ...
                    .catch(error => {
                        console.log("TemplateAdd: api templates NG");
                    });
            }
        },
        //前の画面へ戻る
        returnPage() {
            console.log("TemplateAdd: returnPage");
            this.$router.push({
                name: "template", 
                params: { 
                    selectedDatas: this.selectedDatas,
        ...
        //セレクトボックスの状態操作
        changeInput(id) {
            console.log("TemplateAdd: changeInput");
            if(id == document.getElementById("crop_id").id) {
                ...
                if(this.selected3.length !== 0) {
                    this.selected3 = this.selected3[0].crop_name
                }
            }
        }
    }
